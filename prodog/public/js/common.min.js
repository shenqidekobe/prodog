		...
		switch(typs){
			...
			case "info_install_sold.html"://我的小狗设置出售价格
				...
				$("#soldAmountSaveBtn").click(function(){
					...
					aschCONTRACT(1004,'0',{id:id,amount:amount},function(dos){
						console.info(dos);
					});
				});
			break;
			case "info_install_pair.html"://我的小狗设置配对价格
				...
				$("#pairAmountSaveBtn").click(function(){
					...
					if(amount==''||(Number(amount))>999999999)return false;
					aschCONTRACT(1007,'0',{id:id,amount:amount},function(dos){
						console.info(dos);
					});
				});
			break;
		...
		for(var t in tasks){
			aschHTTP('get','/api/transactions/get',{Id:tasks[i].transactionId},function(rsp){
				...
				if(rsp.success&&rsp.transaction!=null){
					...
					aschCONTRACT(2002,'0',{status:2,id:tasks[i].id},function(dss){
						console.info(dss);
						if(dss.success){
							switch(typs){
							    case "1"://购买小狗
							    	aschCONTRACT(1005,'0',{address:tasks[i].address},function(dos){
							    		console.info(dos);
									});
								break;
							}
