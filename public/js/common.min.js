$(function(){
    ...
    let checkLoginPages="info.html,info_dogs.html,info_dog_detail.html,lottery.html,lottery_yes.html,lottery_ing.html";
    let userInfo={};
    let userBalance=0;
	...
	function saveTask(transactionId,address,type){
		var args=[];
		args.push(transactionId);
		args.push(address);
		args.push(type);
		aschCONTRACT(2001,'0',JSON.stringify(args),function(dss){
			if(!dss.success){
				toast('任务发布失败:'+transactionId);
			}
		});
	}
	var interval=null;
	function countDown(endDate,type) { 
		var now = new Date(); 
	...
	$(window).load(function(){
		switch(typs){
			...
			case "info_dogs.html"://我的小狗列表
				if(!isLogin()){location.href='login.html';}
				interfaceASCH('/info/dogs',{address:getUser().address},function(rsp){
			...
			case "info_dog_detail.html"://我的小狗详情
				...
				interfaceASCH('/fulldogs/'+id,{},function(rsp){
					...
					var html = $(template("DOGDETAIL",dog));
				    $(".content").html(html);
				    $("#give").click(function(){
				    	location.href='success_give.html';
				    });
					$("#sold").click(function(){location.href='info_install_sold.html?id='+id;});
					$("#pair").click(function(){location.href='info_install_pair.html?id='+id;});
				});
			break;
			case "info_install_sold.html"://我的小狗设置出售价格
				if(!isLogin()){location.href='login.html';}
				var id=getUrlParams("id");
				$("#soldAmountSaveBtn").click(function(){
					var amount=$.trim($("#soldAmount").val());
					if(amount==''||(Number(amount))>999999999)return false;
					var args=[];
					args[0]=id;
					args[1]=amount;
					aschCONTRACT(1004,'0',JSON.stringify(args),function(dos){
						if(dos.success){
							toast("设置成功");
							location.href='info_dogs.html';
						}else{
							toast("设置失败");
						}
					});
			...
			break;
			case "info_install_pair.html"://我的小狗设置配对价格
				if(!isLogin()){location.href='login.html';}
				var id=getUrlParams("id");
				$("#pairAmountSaveBtn").click(function(){
					var amount=$.trim($("#pairAmount").val());
					if(amount==''||(Number(amount))>999999999)return false;
					var args=[];
					args[0]=id;
					args[1]=amount;
					aschCONTRACT(1007,'0',JSON.stringify(args),function(dos){
						if(dos.success){
							toast("设置成功");
							location.href='info_dogs.html';
						}else{
			...
			break;
			case "info_install_nickname.html"://设置昵称
				if(!isLogin()){location.href='login.html';}
				var id=getUrlParams("id");
				$("#nameSaveBtn").click(function(){
					...
					args[0]=id;
					args[1]=nickname;
					aschCONTRACT(1003,'0',JSON.stringify(args),function(dos){
						if(dos.success){
							toast("设置成功");
							location.href='info_dogs.html';
						}else{
			...
			case "lottery.html":
				...
				serverHTTP('lottery/next_award',{},function(rsp){
					...
					var endDate = new Date(ds[0],m-1,d);
					endDate.setHours(parseInt(h));
					endDate.setMinutes(mu);
					interval=window.setInterval(function(){countDown(endDate,'lottery');}, 1000); 
				});
			...
			case "market_list.html"://市场列表的狗
				...
				$(document).on('mousedown','.f-click',function(event){
					var nthis=this,pid=$(this).attr('data-pid'),
					par=$(this).attr('data-par'),sid=$(this).attr('data-sid'),cid=$(this).attr('data-cid'),
					val=$(this).attr('data-v'),pbg=$(this).attr('data-popbg'),sho=$(this).attr('data-show');
					switch($(this).attr('data-id')){
						...
						case "select-v":
							var v=$(this).text();
							$(cid).text(v).removeClass('active');;
							$(sho).fadeToggle(300);
							$(pbg).fadeToggle(260);
							$('body').removeClass('overflow-y');
							interfaceASCH('/market/dogs',{isold:1,sort:val.split("_")[0],dir:val.split("_")[1]},function(rsp){
								var data={list:rsp.dogs};
								console.info(data);
								template.helper("getGenerationStr", function(ds) {
									if(ds.generation == 0) {return "一";}
			...
			case "market_order_pay.html"://市场订单支付
				...
				interfaceASCH('/dogs/'+id,{},function(rsp){
					...
					var cuy=data.currency;
					cuy=typeof(cuy)=='undefined'?ASCH_CURRENCY:cuy;
					$("#amount").text(amount+' '+cuy);
					$("#fee").text(MARKET_FEE+' '+cuy);
					$("#total").text((MARKET_FEE+amount)+' '+cuy);
			...
			case "pair.html"://去配对小狗
				if(!isLogin()){location.href='login.html';}
				var did=getUrlParams("did");//我的小狗ID
				var pid=getUrlParams("pid");//配对的小狗ID
				if(pid==null){location.href='info.html';}
				if(did!=''&&did!=null){
					$(".bgc-fff").remove();
				...
				//配对的小狗
				interfaceASCH('/dogs/'+pid,{},function(rsp){
					var dog=rsp.dog;
					$(".tit-fs18").text(dog.nickname);
					$("#amount").text(dog.pairamount+' '+ASCH_CURRENCY);
				});
				...
				$("#pairConfirmBtn").click(function(){
					...
					args[0]=did;
					args[1]=pid;
					aschCONTRACT(1008,'0',JSON.stringify(args),function(dss){
						if(dss.success){
							location.href='success_pair.html?id='+did;
						}else{
							toast('配对失败，请联系管理员');
						}
					});
			...
			case "success_pair.html"://配对成功提示页面
				...
				interfaceASCH('/dogs/'+id,{},function(rsp){
					var dog=rsp.dog;
					if(dog==null||dog.status!=4){
						location.href='info.html';
						return false;
					}
					var starttime=dog.pairstarttime;
					var endDate=datePlus(starttime,Number(dog.pairtime));
					interval=window.setInterval(function(){countDown(endDate,'pair');}, 1000); 
				});
			break;
	...
	var tasks=null;
	$(window).load(function(){
		interfaceASCH('/exe/tasks',{status:1},function(rsp){
			if(rsp.count<=0)return false;
			tasks=rsp.results;
			task.executeTask(queryTransaction);
		});
	...
	function queryTransaction(){
		...
		for(var t in tasks){
			aschHTTP('get','/api/transactions/get',{Id:tasks[i].transactionId},function(rsp){
				console.info(rsp);
				var type=tasks[i].type;
				if(rsp.success&&rsp.transaction!=null){
					//查询到交易信息更新数据,并且处理相关业务数据
					var args=[];
					args[0]=tasks[i].id;
					args[1]=2;
					aschCONTRACT(2002,'0',JSON.stringify(args),function(dss){
						console.info(dss);
						if(dss.success){
							switch(typs){
							    case "1"://购买小狗
							    	var args2=[];
							    	args2[0]=tasks[i].address;
							    	aschCONTRACT(1005,'0',JSON.stringify(args2),function(dos){
							    		console.info(dos);
									});
								break;
							}
